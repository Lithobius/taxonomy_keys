...
crabs db_merge --output 20241010_12s_crabs-merge.fasta --uniq yes --input 20241010_mitofish_12s-filter.fasta 20241010_12s_fish-ncbi-filter.fasta 20241006_12s_outgroups-ncbi-filter2.fasta 20240212_12s_outgroups-ncbi-filter2.fasta #20230905_12s_ncbi-outgroups_crabs-ready.fasta

# 4: in silico pcr
# 12S	MiFish-U, FW:GTCGGTAAAACTCGTGCCAGC, R:CATAGTGGGGTATCTAATCCCAGTTTG
crabs insilico_pcr --input 20241010_12s_crabs-merge.fasta --output 20241010_pcrout.fasta --fwd GTCGGTAAAACTCGTGCCAGC --rev CATAGTGGGGTATCTAATCCCAGTTTG --error 6
...
crabs pga --input 20241010_12s_crabs-merge.fasta --output 20241010_pgaout.fasta --database 20241010_pcrout.fasta --fwd GTCGGTAAAACTCGTGCCAGC --rev CATAGTGGGGTATCTAATCCCAGTTTG --speed medium --percid 0.9 --coverage 0.9 --filter_method relaxed

# 5 assign_tax
# taxonomy assignment by accession number
# --missing for custom names, same format as lineage input
crabs assign_tax --input 20241010_pgaout.fasta --output 20241010_taxassigned.tsv --acc2tax nucl_gb.accession2taxid --taxid nodes.dmp --name names.dmp
...
crabs dereplicate --input 20241010_taxassigned.tsv --output 20241010_dereplicated.tsv --method uniq_species
...
crabs seq_cleanup --input 20240310_dereplicated.tsv --output 20241010_dbcleaned.tsv --discard 20241010_discardedseq.tsv --minlen 20 --maxlen 25000 --maxns 5 --enviro yes --species no --nans 7
