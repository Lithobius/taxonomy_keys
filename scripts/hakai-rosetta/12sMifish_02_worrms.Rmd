...
# in case read in
fish2 <- read_csv(here(rosetta, 'taxize', '20220727_mifish_taxonomy-full.csv'))[-1]
```
...
# match freshwater-only records
fresh_gen <- fish_records_gen %>%
  filter(rank == 'genus' & freshwater_only == 1) %>%
  select(taxa_query, worms_sciname, worms_aphiaid, rank,
         phylum, class, order, family, genus,
         status, unacceptreason, freshwater_only, terrestrial_only) %>%
  rename(genus_split = taxa_query) %>%
  right_join(fish_search3) %>%
  filter(!(is.na(worms_sciname))) %>%
  select(!(c(worms_query, genus_split))) %>%
  rename(species_verbatim = query_verbatim,
         found_taxa = query) %>%
  left_join(fish_tol) %>%
...
# add freshwater only to final
fish2 <- fish2 %>%
  filter(!(is.na(freshwater_only) & is.na(terrestrial_only)))%>%
  bind_rows(fresh_gen) 

#
fish_records_gen2 <- fish_records_gen %>%
  filter(!(rank == 'genus')) %>%
  select(taxa_query, worms_sciname, worms_aphiaid, rank,
         phylum, class, order, family, genus,
         status, unacceptreason, freshwater_only, terrestrial_only) %>%
  left_join(fish_search3, by  = c('taxa_query' = 'query')) %>%
  select(!(c(worms_query, genus_split))) %>%
  rename(species_verbatim = query_verbatim
         ) %>%
  left_join(fish_tol) %>%
...
# bind back in
fish2 <- fish2 %>%
  bind_rows(fish_records_gen2)

write.csv(fish2, here(rosetta, 'taxize', '20220729_mifish_taxonomy.csv'))
```
...
# Still still not found

```{r notfoundstill}
fish_outstanding <- fish_search3 %>%
  filter(!(query %in% fish2$taxa_query))

fish_outstanding2 <- midori_fish %>%
  filter(!(query %in% fish2$taxa_query))

# write out
write.csv(fish_outstanding2, here(rosetta, 'problems',
                                  'not_found', '20220729_mifish_notfound.csv'))
```
