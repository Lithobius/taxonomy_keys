...
library(stringr)
library(taxize)
#library(fuzzyjoin) # i think this was from an older version?
library(here)
...
annelid_notfound <- annelid %>%
  ...
  filter(!(taxa %in% annelid_gbifid2$taxa_query |
           ...
           taxa %in% annelid_worms3$taxa_query |
           taxa %in% annelid_tol2$taxa))

#write out not-found
write.csv(annelid_notfound, here(rosetta, 'problems', '20220624_annelid_not-found.csv'))
...
write.csv(annelid_nf_worms, here(rosetta, 'problems', '20220624_annelid_worms-tol-conflict.csv'))
```
...
arthropod_search <- arthropod %>%
  ...
  select(c(taxa, unique_name, phylum, family)) %>%
  #include replacements from tol and any overhanging taxa
  mutate(taxa_query = coalesce(unique_name, taxa)) %>%
  relocate(taxa_query) %>%
  #remove any known introduction from tol
...
# worms classification
arthropod_worms_classify <- classification(arthropod_worms2$worms_aphiaid, db = 'worms')

# make and rotate dataframe 
arthropod_worms_classify2 <- map_dfr(.x = arthropod_worms_classify, ~ data.frame(.x), .id = 'worms_aphiaid') %>%
  distinct() %>%
  pivot_wider(id_cols = worms_aphiaid, names_from = rank, values_from = c(name, id)) %>%
  rename_with(~ str_replace(.x, 'name_', '')) %>%
  rename_with(~ str_replace(.x, 'id_', 'wormsid_')) %>%
...
arthropod_worms3 <- arthropod_worms2 %>%
  left_join(arthropod_worms_classify2) %>%
  filter(kingdom == 'Animalia') %>%
  janitor::remove_empty() %>%
  #select only desired taxonomic levels and their IDs
  ## note subgenus was empty here but it probably shouldn't be
...
#write full
write.csv(arthropod2, here(rosetta, 'taxize', '20220624_arthropod_taxonomy-full.csv'))
...
arthropod_notfound <- arthropod %>%
  ...
  filter(!(taxa %in% arthropod_gbifid2$taxa_query |
           ...
           taxa %in% arthropod_worms3$taxa_query |
           taxa %in% arthropod_tol2$taxa))

#write out not-found
write.csv(arthropod_notfound, here(rosetta, 'problems', '20220624_arthropod_not-found.csv'))
...
#write out worms issues
write.csv(arthropod_nf_worms, here(rosetta, 'problems', '20220624_arthropod_worms-tol-conflict.csv'))
```
...
# 2: worms
chordata_worms <- get_wormsid_(sci_com = chordata_search$worms_query)
# get_wormsid_ gives back null and length 0 elements
## first remove with compact and discard then map_dfr
...
# worms classification
chordata_worms_classify <- classification(chordata_worms2$worms_aphiaid, db = 'worms')

# make and rotate dataframe 
chordata_worms_classify2 <- map_dfr(.x = chordata_worms_classify, ~ data.frame(.x), .id = 'worms_aphiaid') %>%
  pivot_wider(id_cols = worms_aphiaid, names_from = rank, values_from = c(name, id)) %>%
  rename_with(~ str_replace(.x, 'name_', '')) %>%
  rename_with(~ str_replace(.x, 'id_', 'wormsid_')) %>%
  mutate(worms_aphiaid = as.numeric(worms_aphiaid)) %>%
...
chordata_worms3 <- chordata_worms2 %>%
  left_join(chordata_worms_classify2) %>%
  filter(kingdom == 'Animalia') %>%
  janitor::remove_empty() %>%
  #select only desired taxonomic levels and their IDs
  ## note subgenus was empty here but it probably shouldn't be
...
#write full
write.csv(chordata2, here(rosetta, 'taxize', '20220624_chordata_taxonomy-full.csv'))
...
chordata_notfound <- chordata %>%
  ...
  filter(!(taxa %in% chordata_gbifid2$taxa_query |
           ...
           taxa %in% chordata_worms3$taxa_query |
           taxa %in% chordata_tol2$taxa))

#write out not-found
write.csv(chordata_notfound, here(rosetta, 'problems', '20220624_chordata_not-found.csv'))
...
#write out worms issues
write.csv(chordata_nf_worms, here(rosetta, 'problems', '20220624_chordata_worms-tol-conflict.csv'))
```
...
# worms classification
mollusca_worms_classify <- classification(mollusca_worms2$worms_aphiaid, db = 'worms')

# make and rotate dataframe 
mollusca_worms_classify2 <- map_dfr(.x = mollusca_worms_classify, ~ data.frame(.x), .id = 'worms_aphiaid') %>%
  pivot_wider(id_cols = worms_aphiaid, names_from = rank, values_from = c(name, id)) %>%
  rename_with(~ str_replace(.x, 'name_', '')) %>%
  rename_with(~ str_replace(.x, 'id_', 'wormsid_')) %>%
  mutate(worms_aphiaid = as.numeric(worms_aphiaid)) %>%
...
mollusca_worms3 <- mollusca_worms2 %>%
  left_join(mollusca_worms_classify2) %>%
  filter(kingdom == 'Animalia') %>%
  janitor::remove_empty() %>%
  #select only desired taxonomic levels and their IDs
  ## note subgenus was empty here but it probably shouldn't be
...
#write full
write.csv(mollusca2, here(rosetta, 'taxize', '20220624_mollusca_taxonomy-full.csv'))
...
mollusca_notfound <- mollusca %>%
  ...
  filter(!(taxa %in% mollusca_gbifid2$taxa_query |
           ...
           taxa %in% mollusca_worms3$taxa_query |
           taxa %in% mollusca_tol2$taxa))

#write out not-found
write.csv(mollusca_notfound, here(rosetta, 'problems', '20220624_mollusca_not-found.csv'))
...
#write out worms issues
write.csv(mollusca_nf_worms, here(rosetta, 'problems', '20220624_mollusca_worms-tol-conflict.csv'))
```
...
# worms classification
ech_platy_nem_worms_classify <- classification(ech_platy_nem_worms2$worms_aphiaid, db = 'worms')

# make and rotate dataframe 
ech_platy_nem_worms_classify2 <- map_dfr(.x = ech_platy_nem_worms_classify, ~ data.frame(.x), .id = 'worms_aphiaid') %>%
  pivot_wider(id_cols = worms_aphiaid, names_from = rank, values_from = c(name, id)) %>%
  rename_with(~ str_replace(.x, 'name_', '')) %>%
  rename_with(~ str_replace(.x, 'id_', 'wormsid_')) %>%
  mutate(worms_aphiaid = as.numeric(worms_aphiaid)) %>%
...
ech_platy_nem_worms3 <- ech_platy_nem_worms2 %>%
  left_join(ech_platy_nem_worms_classify2) %>%
  filter(kingdom == 'Animalia') %>%
  janitor::remove_empty() %>%
  #select only desired taxonomic levels and their IDs
  ## note subgenus was empty here but it probably shouldn't be
...
ech_platy_nem2 <- ech_platy_nem_search %>%
  #select(species) %>%
  #rename(query = species) %>%
  left_join(ech_platy_nem_worms3) %>%
...
#write full
write.csv(ech_platy_nem2, here(rosetta, 'taxize', '20220624_ech_platy_nem_taxonomy-full.csv'))
...
ech_platy_nem_notfound <- ech_platy_nem %>%
  ...
  filter(!(taxa %in% ech_platy_nem_gbifid2$taxa_query |
           ...
           taxa %in% ech_platy_nem_worms3$taxa_query |
           taxa %in% ech_platy_nem_tol2$taxa))

#write out not-found
write.csv(ech_platy_nem_notfound, here(rosetta, 'problems', '20220624_ech_platy_nem_not-found.csv'))
...
#write out worms issues
write.csv(ech_platy_nem_nf_worms, here(rosetta, 'problems', '20220624_ech_platy_nem_worms-tol-conflict.csv'))
```
...
# worms classification
small_phyla_worms_classify <- classification(small_phyla_worms2$worms_aphiaid, db = 'worms')

# make and rotate dataframe 
small_phyla_worms_classify2 <- map_dfr(.x = small_phyla_worms_classify, ~ data.frame(.x), .id = 'worms_aphiaid') %>%
  pivot_wider(id_cols = worms_aphiaid, names_from = rank, values_from = c(name, id)) %>%
  rename_with(~ str_replace(.x, 'name_', '')) %>%
  rename_with(~ str_replace(.x, 'id_', 'wormsid_')) %>%
  mutate(worms_aphiaid = as.numeric(worms_aphiaid)) %>%
...
small_phyla_worms3 <- small_phyla_worms2 %>%
  left_join(small_phyla_worms_classify2) %>%
  filter(kingdom == 'Animalia') %>%
  janitor::remove_empty() %>%
  #select only desired taxonomic levels and their IDs
  ## note subgenus was empty here but it probably shouldn't be
...
#write full
write.csv(small_phyla2, here(rosetta, 'taxize', '20220624_small_phyla_taxonomy-full.csv'))
...
small_phyla_notfound <- small_phyla %>%
  ...
  filter(!(taxa %in% small_phyla_gbifid2$taxa_query |
           ...
           taxa %in% small_phyla_worms3$taxa_query |
           taxa %in% small_phyla_tol2$taxa))

#write out not-found
write.csv(small_phyla_notfound, here(rosetta, 'problems', '20220624_small_phyla_not-found.csv'))
...
#write out worms issues
write.csv(small_phyla_nf_worms, here(rosetta, 'problems', '20220624_small_phyla_worms-tol-conflict.csv'))
```
