...
chordata_records <- chordata_records %>%
  ...
  relocate(c(valid_name, worms_aphiaid), .before = original_aphiaid) %>%
  # rank lowercase
  mutate(rank = tolower(rank))


chordata_search2 <- chordata_search[1:500,]

#3: gbif+bold ids
# now we'll search bold
chordata_bold <- get_boldid_(sci = chordata_search2$taxa_query, 
                         includeTree = TRUE,
                         divison = 'Animalia')
#bind up lists into a dataframe
...
# GBIF id
chordata_gbifid <- get_gbifid_(sci = chordata_search2$taxa_query, rows = 1)
