---
title: "COI_03_hakai-db_intermediate-updates"
...
hakai_rosetta <- read_csv(here('latest-keys', '20220725_coi_rosetta_draft.csv'))

# only need select rows; the rest can be blank for now
rosetta_to_match <- hakai_rosetta %>%
  select(found_taxa, identification, rank,
...
not_matched <- read_csv(here(problems, 'other', '20220721_coi-notmatched.csv'))

to_match <- not_matched %>%
  rename(verbatim_identification = lca_taxon_verbatim,
         identification = lca_taxon) %>%
  left_join(rosetta_to_match)

matched <- to_match %>%
  filter(!(is.na(found_taxa)))

still_not_matched <- to_match %>%
  filter(is.na(found_taxa)) %>%
  select(identification, verbatim_identification)

```

```{r search still not matched}
nm_resolved <- tol_resolve(still_not_matched$identification)
...
to_match_resolved <- to_match %>%
  ...
  select(!(kingdom))
```
## removing still not found

```{r}
hakai_rosetta <- hakai_rosetta %>%
  filter(!(is.na(found_taxa)))
```
...
```{r update rosetta}
hakai_rosetta <- hakai_rosetta %>%
  bind_rows(to_match_resolved)

# write in update!
write_csv(hakai_rosetta, here('latest-keys', '20220902_coi_rosetta_draft.csv'))
...
```{r update fresh-terrestrial}

all_freshterr <- read_csv(here('latest-keys', '20220721_freshwater_terrestrial.csv'))

all_freshterr <- all_freshterr %>%
  bind_rows(to_match_resolved) %>%
  filter(terrestrial_only == 1 | freshwater_only == 1)

write_csv(all_freshterr, here('latest-keys', '20220725_freshwater_terrestrial.csv'))
