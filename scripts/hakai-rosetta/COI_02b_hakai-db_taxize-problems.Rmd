...
annelid_nf_worms <- read_csv(here(tol_conflict,
  ...
  #clean tol_sciname column
  mutate(tol_sciname = str_remove_all(tol_sciname, " \\(.*\\)")) %>%
  mutate(tol_sciname = trimws(tol_sciname))
```
...
annelid_tol_worms3 <- annelid_nf_worms %>%
  filter(!(tol_sciname %in% annelid2$tol_sciname)) %>%
  select(taxa_query, ott_id, is_synonym, flags, tol_sciname, family)
...
annelid_family_fix <- annelid2 %>%
  ...
  mutate(found_taxa = tol_sciname)


# bind generic resolutions
annelid2 <- annelid2 %>%
  bind_rows(annelid_genera_fix, annelid_family_fix)
...
arthropod_genera_fix <- arthropod_nf_worms %>%
  select(taxa_query, ott_id, is_synonym, flags, tol_sciname) %>%
  filter(!(tol_sciname %in% arthropod_records$taxa_query)) %>%
  # extract genus names so we can test them
  separate(tol_sciname, into = 'genus_split', " ", 1, 
           remove = FALSE, extra = 'drop') %>%
  left_join(arthropod_genera, by = c('genus_split' = 'genus')) %>%
  filter(!(is.na(found_taxa))) %>%
  select(!(genus_split)) %>%
  mutate(rank = ifelse(tol_sciname == found_taxa, 'genus', 'species')) %>%
  mutate(found_taxa = ifelse(tol_sciname == found_taxa, found_taxa, tol_sciname))

### all arthropod families were NA in this round
# family-level search
## typically this will be terrestrial or freshwater-only families
arthropod_family <- arthropod_nf_worms %>%
  filter(!(tol_sciname %in% arthropod2$tol_sciname)) %>%
  select(taxa_query, ott_id, is_synonym, flags, tol_sciname, family)


#arthropod_family_fix <- arthropod2 %>%
#  filter(rank == 'family') %>%
#  select(!(c(ott_id, tol_sciname, flags, is_synonym, taxa_query, found_taxa))) %>%
#  distinct() %>%
#  right_join(arthropod_tol_worms3) %>%
#  filter(!(is.na(worms_aphiaid))) %>%
  # these are all specific level IDs as generic level was resolved earlier
  ## rank should reflect this
#  mutate(rank = recode(rank, family = 'species')) %>%
...
# add to arthropod found list
arthropod2 <- arthropod2 %>%
  bind_rows(arthropod_genera_fix#, arthropod_family_fix
            )
