...
ncbi_acc_all <- ncbi_acc %>%
  ...
  mutate(remove_me = case_when(found_taxa %in% c('Antennarius', 'Acropoma japonicum', 'Symphodus ocellatus',
                                                 ...
                                                 'Gymnura', 'Sphyraena japonica', 'Triglops jordani', 'Labiobarbus leptocheilus', 'Barbus', 'Thunnus alalunga', 'Nothobranchius nubaensis', 'Arothron stellatus', 'Geophagus brasiliensis', 'Inu', 'Catostomus occidentalis') & 
                                 status != 'accepted' ~ 'yes')) %>%
  filter(is.na(remove_me)) %>%
  add_count(accession, name = 'n_acc') 
