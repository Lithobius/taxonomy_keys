...
ncbi_fish_gene <- ncbi_fish %>%
  ...
  mutate(genome = ifelse(str_detect(desc_verbatim, 'complete genome|partial genome|partiale genome|mitochondrion sequence|mitochondrial DNA|genome assembly|mitochondrial genome|mitochondrial sequence|mitochondrionial geneome|mitochondrial genes'), 
                         "yes", "no")) %>%
  mutate(small_subunit = ifelse(str_detect(desc_verbatim, '(small subunit)'), 'yes', 'no')) %>%
...
fish_ncbi_result <- ncbi_organism %>%
  ...
  # ncbi_sp complexes
  mutate(ncbi_sp = str_replace_all(ncbi_sp, "( aff )|( cf )", " ")) %>%
  ...
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( type )|Type |( Type)$|( type)$|( n )|( new)$|( nov )|(nomen nudum)|Haplotype|Lineage|Unidentified|uncataloged|uncatalogued")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " genotype| ecotype| morphotype| voucher| specimen| popvariant| museum|( et al)| field number|( sensu)| lineage| hybrid|( wild)|( var)$|(Natural )|( breed)|(mutant)$| except |( except)$| lot | like |( like)$| from |( lot)$| almost| Personal| individual|( de )| Vouch| common|population| nearly|( Lot)$|paralectotype|collection|Collection|( nearly )|Individual|museum|Museum|Petshop|Hatchery|(Hatch)$|( in )|between|( many)$|( note )")) %>%
  ...
  # ways to say "fish"
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( efish)$|(eFish)$|(Fish)$||( Fish )|( fish )|( fish)$|(Fisk)$|(Ichthyology)|(Ich)$|(IDEAFish)|USNMFish|NMFish|MCZIch|MZict|MiFish|( ZMUCFisk)|FISHCARD| Fishec|Hatchery|OSUMZFish|SLSCFish|FishIRAN|Lifish|SCFish|(Ichthyology)|ichthyology|(FIchthyology)|Ichthyology|Ichthyological|CASIch|( Univ)$|MZUSPskel|UMass|AQAHSP|Office")) %>%
  # commentary
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(We noticed some discrepancies)")) %>%
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  ...
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "mitochondrial|mitochondria|mitochodrial|mitochondral|Mitochondrial| cytochrome|Cytochrome| oxidase|Oxidase| subunit|Subunit|subuinit|(cytb)|III| intergenetic| spacer| region| dehydrogenase| ribosomal| molecule| Clonal|( DNA No)| in bony fishes| gynogenic|natural | chloroplast| encoding|  product| sequence|Valsequence| ubiquinone| oxidoreductase| NADH| synthase| cyt b| cytb| Cytb| ctyb|cybt| mitchondrial| ATPase|ATP| dkisp|ssequence|RNA| Icox| synthetase| Bus| cytocrome| cytchrome|COX|( cox)$|( cyt)$| assembly| organelle| putative| duplication| cds| scds| lcds| Icds|( cyp)$| cyp |COSeq| Taq| cox |( end)$|(apocytochrome)| intron| fragment|seqeucne|polymorphism|intergenic|NADH|Dehydrogenase| Synucal| adenine |nicotinamide|dinucleotide")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z][A-Z]Seq)|(allele)$|( Amino)")) %>%
  # Methods
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Miseq)| RNAThr|(Zap Express)|( Photo)$| environmental| sample|proteobacterium| biomaterial| tissue| control| contol| sequence| remnant|Bacillariophyta| transcript| artificial| culture|bacterium|enrichment|Riomix|mixedeggs|subunit|cellline|(MiFish)| plastid|( Lab )| number|(no number)|Number| system| incomplete|ASMBLY|nuclear| pseudo|minicircle| copies|compelete| domain|hydrophlox|( repeat)|( unit )|truncate|( bp )|( copy)$|( copy )|( gel )|DoxMito")) %>%
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  ...
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  # methods leftovers
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "ofunit|( of unit)|aavariant|acopies|( unit)$|MLR| wicor| parial|( product)$")) %>%
  # morphology
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( spot )|( spot)$|( Spotted)|( barred)|( dwarf)|( large )|( large)$|( small )| Cribrimorphlike|( pinnules)|( crescent)$|( spiral)$|(collar)|( Laser)|( Super)$|( special)$|( Illuminator)|( long )|( Long )|( short)$|( short )|( Last)| stripe | blotch| deep |( scute)$| scute | upper | left |( left)$| shallow|Longtailed|(Gluteus)$| Pearl|(Fly)$|( papilio )|( slender)|( variant)|Dotted|Lined|(Big )|Upper|( Horn)$|(Crystal)$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Head kidney)| Liver|(bulky head)|( brain)|( muscle)|( Muscle)|( Tissue)|(Spleen of)|(gynotic )| infected|(prey of)|( larva)|( eyes)$|( internal)$|( internal )| nudepalp|(palp)$|(Fin)$|( gill )|symbiont|thioautotrophic|unicuspid|scraper|bullet")) %>%
  # color
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Black)| black| chocolate|( Green)|( green)|( Red)|(Red)$|( red )|(  red)$|( rosy)|(Verde)$| orangecollar| Orange| orange |( orange)$|( yellow)|( Yellow)|( yellowback)|( yellowcollar)|( yellowpatch)|( Gold)|(Gold)$|( albino )|( white)|( White)| blond|( neon)$|( Aoi)|( Blue)|( blue )|( tan )")) %>%
  # other animal names
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Tuna)|( Perche)|( blackcarp)|( carp gudgeon)|( Eel)|ironfish|( Salmon)|(Mackerel)| Puma Loach|( Porcupine)| sawfin shiner| brassy jumprock|( chimpanzeecA)|( gibbongA)|( ex Sooretamys angouya)$|( Phantom)|(barbel steed)| goldfish| LAcanthogobius|Amblyrajaradiata|Amblyrajajenseni| Cotcog|Dmacr|Gbar|Acinipo|Aoxyrinchus|Aalabamae|Dolopichthyslongicornis|Abbottinarivularis|dClupeidae|americaneel| Fish|Xenentodoncancila|TXenocephalus|Xargentea|Xfangi|Xcap|Xvar|sunflower|Zaccoplatypus|(Sturg)$|Acebrev|Aceten|Anoxypristiscuspidata|Antimorarostrata|(Goby)$|Artediellusatlanticus|Macrourusberglax|Bpeteny|Bpelopon|Bpaludin|Bpaludino||Magnisudisatlantica|(AuxisThrespuram)$|( duck)$|partridge|longnosegar|Lcyanellus|Mnegrosensis|Malacorajaspinacidermis|Malacosteusniger|Mallotusvillosus|Pseudafer|Ptenuis|Bcanis|Lkasmira|Lmonostigma|QLut|Lycenchelyspaxillus|Lygro|Lycenchelyspaxillus|Lycodesluetkenii|Lycodesmcallisteri|Lycodespaamiuti|Lycodespallidus|Lycodesseminudus|Lycodessquamiventer|Lycodesgracilis|Lycodesvahlii|Lycodonusflagellicauda|Lycodonusmirabilis|roundgoby|Nezumiabairdii|Ncrysoleucas|Obladamelanura|(Ohat)$|(Obon)$|(Oarg)$|OCstomias|Oncorhynchusgorbuscha|Oneirodesheteronema")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Pholisfasciata|Lycodespaamiuti|Lycodeseudipleurostictus|Lmelanogaster|Omykiss|OCstomias|Ioxy|( salmon )|Oncorhynchusgorbuscha|Sebastesmentella|Sebastesnorvegicus|Tmyops|( duck)$|Pmag|yoshinogochi| Flounder| SPlaternarius|Hippoglossoidesplatessoides|Pinnesi|Paraliparisbathybius|Paralipariscopei|Paraliparisgarmani| partridge|Pnigromaculatus|Ssaurus|Parablenniussanguinolentus| Ennans|Spondyliosomacantharus|(Saca)$| Lerythracanthus|Poromitracapito|Bfrits|Bcyccholor|Bcanis|Bcaninus|Cmulti|Ptenuis|Synbia|Stenu|Styph|( Sabas)|Synaphobranchuskaupii|Bcyccolor|Bcycsper|Bcystru|Bstrum|Beuboi|Aana|AanaDinar|AanaKirk|AanaNazilli|( Asur)|( Atran)|Aapo|( Adis)|( Arich)|( Amen)|CorsarYen|CSSob|Gaidropsarusargentatus|(Hypu)$|( Afas)|Avlad|Coryphaenoidesrupestris|CorsarMes|Corsarneit|Anspin|Avirescens")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Streptococcus|AuxisThrespuram|Bathyrajaspinicauda| Borostomiasantarcticus|Cbispinosa|Centroscymnuscoelolepis|Ecseniuslineatuspetshop|Gaidropsarusensis|Holtbyrniamacrops|Johniusplagiostoma|Lampanyctusintricarius|Lxanthochilus|Lobsoletus|Lerythropterus|Laspera|Liparistunicatus|Lepidioneques|Lycenchelyspaxillus|Lycodesseminudus|Lycodespallidus|Malacorajaspinacidermis|Mallotusvillosus|Melanogrammusaeglefinus|Micromesistiuspoutassou|Mgroculis|Spvolitans|Polyipnuspolli|Pristispectinata|Pristispristis|PRspas|Psednosmicruroides|Pdispar|Scopelosauruslepidus|Solspi|Somniosusmicrocephalus|( Scus)$|Sparisomacretense|( Smok)$|( Stib)$|( Szyg)$|dSprattusTallinn|( pig )|( pig)$|( Cbur)|(Corn)$|(Clim)$|Cgac|(Cuma)$|(Caur)$|Cpul|CorsarNeit|Cmiv|Cvaregiatus")) %>%
  
  # location modifiers
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Lake )|( Lake)|( cave)$|( spring)$|( springs)$|( river )|(river)$|( River)|( stream)|( Creek)|( creek)|( near )|( sea)$|( sea )|( Sea)$|RedSea|( Pool)|( pool)$|(Pool)$|( City)|( reef )|( reef)$|( aquatic)$|( farm)|( Plateau)|( Bay)$|( Bay )|( Agua)|( Ocean)|( Isl )|( Isl)$|( Park)$|( Lake)$|(Gulf)$|Delta|( Bay)$|( stone)$|( Stony)|( rock )|deepwater|tributary|Drain|Lagoon|( bay)$|( Isls)$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Aquarium)| Province| province|Station| state |(state)$|(plantation)$| country|coutnry|continental|National|( Camp )|( Camp)$|( camp)$| prefecture")) %>%
  ...
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  # location names
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Adan)$|Amazonas| Amazon|Ampden| Adirondacks|( Australia)$|(Great Australian Bight)| Azores| Andai| Akaebi| Acla| Argis| Austria|Asaraki|Aburatsubo|Antalaha| Albanien| Almenara| Araguaia| Ariv|(Amur)$| Apom| Atlantic| Alberca|( Almond)|AROcr|ATHcr| Apure|Allens| Alshehri|Acarareco|Amargosa|(ArutVolga)|(AlosaVolgaDelta)|(Astrakhan)|Aichi|Atachi|Asalda|Ayong|Aogaki|Akita|Aomoro|Akkeshi")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Benin)|Blanes|(Brazil)|( Bidou)|( Bissiang)|( Bioko)|( Bundu)|Bambama|Biscayne|( Bush)$|(Bitter)|(Bivavotou)|(Bioko)| Beforana| Betampona|( Borneo)| Beysehir| Betari|brooksilverside|( Belisario)| Baldy|( Burn)$| Braid| Brox| Burdiehouse|Bangladesh|(Blyth)$|( Banco)$|Bellwood|Bifounda|Biafra| Bear|BigSmokey|Boise|Belize|Balidian|Balhash|Bollinger|Boone|Beira| Biwa|Benue|Bissau|Beana|Boloby")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Cape )|Clutha|China|Croatia| Cunha| Carombe|( Carib)$|( Cap Esterias)|(Chiba Museum)|( Chiba)$|(CabotCenter)| Continental| Cayman| Caracol| Calibasis| Chiriqui| Collier|( Caroline)| Contulmo| Chao Phraya|( Cana)| Columbia| Corrantijn| Churashima| Charo| Cauvery| Clearwater| Cuyuni|| Carson|Cooscoosro| Calinia| CaliniaPendleton|Chitose|CaspianSea|Catlins|(Costa Rica)|CanoRey|Churaumi|Chittar|Churaumi|Changxing|Chisanza| Corint|(Chiba)|Chippewa|Cooper|Cudgewa|Clarence")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Camcomy")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Darling| Dettai|Dizangue|Daimyojin| Dinar|Dibeng|( Deja)$|(Disc)$|daAlosa|daAzov|Dzangu|Dickerson| Doce|Dhupeinen|Dinoville|dVolga|Damine|Donodagawa")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Esmeraldas)$|Esterias|( Elon)|Ehime| Eric| Eritrea|Ecuadorian| Farias|(Fukuoka Ima)|Fukushima|Funge|Fourche|Fukui| Fuji|FAKUFac|Florida|Fispet")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " Girona| Gekara|(Greece)$|Guangdong|Guangxi|Guizhou|( Garcia)| Guinea| Ginbuna| Guyana| Gamba| Guama| Gifu|Galapagos| Gough| Gonave| Gaus|Guatamala|Gijon|Gabon|greaterredhorse|GreatTasmania|Geum")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Hoyosa)|hauchiwayarimimizuhaze|hiironagamimizuhaze| hosomimizuhaze| Hanel|Hong Kong|( Hino)$|Haltae|(Hypso)$|Hyogo|(Hypu)$|Hiruzen|Hokkaido|( Hoki)|( Hiko)|Huacamayo|Hiping")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " Ivoloina|( Israel)| Ikorodu| Inji|Ireng| Ijebu Ode|(Italy)|Iriomote| Izumihaze|(Iguassu)|ichimonjimimizuhaze|( Indo)$|( Ise)$|( Ito)$| Indian| India|Ichinosegawa|Iquitos| Inspire|Indonesia|Ishioka| Inabe|Iwate|Ishigaki|Ishikawa| Ibia|Ichani| Ibic|Ikange|( Inga)")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " JangLiaw|Japan| Jaffa|(Jian Yang)|Jiaozhou|(Jari)$|Janhauaca")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Kirk)$|( Kirk)|( Kocher)|(Kipar)|( Kami)|Kakuda|Kalwungwishi|( Kribi)|(Kuckuk)$|(Katsutoshi Watanabe)|(Komonyatsushihaze)|(Khukh)|(Kinbuna)| Karianga|(Kapinar)| Kieth| Kisangani| Kongou|Kansas|Kumba|Kaucos| kamahiremimizuhaze|kimairamimizuhaze|komuginagamimizuhaze|( Kochi)|Korea|Kyoto|Kamaishi|Kouchi|Keller|( Kagoshima)|Kamenny|Komono|Kasuga|Kawanishi|Kutami|Kayumi|Kanagawa|Kalungwishi")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Lagos)|( Laos)|(Lebanon)$|( LoireLac)|(Liang Cao)| Lleida||(Lobaye)| Lucien|Lakes|(Luo)$|( Lakshadweep)|( Little )|( Lola)$| Leus|LagCat|Liling|(L Rocha)|Lambarene| Larak|Lengeh|Lambarene|Libreville")) %>%
    mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Muerz)|( Malibo)||(Midgleys)|(Murray)|Makuya| Mazinzi| Mfulgida| MinasBasin| MagdalenIs|( Mai)|( Mic)$|(Micnat)| MicRamrun| Mitsem| Madimba|(Madeira)| Mundemba| Machinda| Makuya|Mexico| Montenegro| Madagascar| Manjavacas| Makira| Martinique| Maranho| Moabi| Myanmar| Miya| Malinau| Manombo| Mahanara| Munday| Massana|(Masatoshi Nakamura)|( Magdalena)| Mongagu| Maroni| Martinso| Macacu| Maroa| mizuhikinagamimizuhaze| Malawi| Marten| Minztita|Misaki| Moloya|Mediterranean|Monzon|Mabuchi|Mkongo|Miami|Melanzona|Maine")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Michmoret)| Mahesh|(Mann)$| Maui|Momobe|Maerewhenua|( May)$|Miyazawa|Miyazaki|Miyagi|Miura|Moroiso|Malebo|Massana|Mamigasaki| manombo|Macacu|Mataura|( Miya)$|Miyame|Miyagawa|Miyashitagawa|Miyanoura|Mvang|Matomb|Matsuo|Marumori|( Manna)|Mitogawa|( Mie )|(Mie)$|(Mogami)|Midgleys|Murray|Matsushima|Miyadu|Mundemba|Moabi|Maramec|Mikkabichouhonmachi|Madeira|Miclei|Mediterranean|Masatoshi|Mayuka|Minago|Midway|Mahanara|Manombo|Moheji|Magdalena|(Maori )")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Nazilli)$| Ndombe|Ntaylori|Nakajima| Ngombe| Nguru| Ndop| Nakuru| Nakabo|( Negro)$| Nevada| Nakabo| Nigeria| Nanay|( Napo)$|Nkumbura|Nagasaki|Nkapa|NewChile|(New Caledonia)|Nagao|Numba|Numazu|Namdae|Nanegawa|Nakamura|Nazilli|Ndokama")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " Orinoco|Odovii|Oreti|Otaru|Ohunato|Onagawa|( Oba)$|Osumi|Onjuku|Obitsu|Onoda|Okarito|Otowagawa|Oirase|Oyodo|( Oyo)$|ochokonagamimizuhaze|oguronagamimizuhaze|Okinawa|Oota|Okuhata|Obilly|Okayama|Ohozora|Ooe|Ooshiro")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " Patrocnica| Patchogue| Pachon|(Paris)| Pampang| Patrocnica| Patchogue| Pachon|(Pingxiang)| Paraguay| Paraguaza|( Peru)|( Panama)| Piedade| Puerto Ayucucho|Paraiba do Sol|PePoAs| Paul| Platanera| Principe|Philippines| Pendleton| Pazo|Palime|PlumCrk|Paraiba|(Panama)$| Poach|Qingdao|Poubara|Putian|Portugal|(Pacific)$|Panacea|Pueblo|Phodia|Pirapo|Paragua|Pegridiri|Police|Poolburn")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Qianshao|(Rio Chirgua)| Ranomafana| Rakhine|( Rio)$|(Rio Sao Francisco)|(Rio de Toca)|(Rio Supamo)| Rydak|Russia|RioMen|RioClara|RioGre|Rocha|(Rio Baia)|(Rio Parana)|RParana|Ravelobe|Rubicon")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Suji)|( Suiyo)|(Smith)$|( Stevens)|(Shibukawa)|Seita| Salsburger| Surinam| Stepian| Stepien|(Sanya)|(Spain)$|( sagami)|(Santa Clara)| Scorff| Sahiler|Seomjin| Sambava| Samou|Shizuoka|(Serra do Cip)|(sPengze)|Sao Joao| Saint|(Solim)$| Skalkaho| Sisi| Shantou| Sri Lanka|(Sao Tome)|Sarem|Santo|Santiago|Shimane|Saitama|Shibetsu|Shiranuka|Sindara|Shikotsu|Synulou|Synufus|Sacus|( stri)|Shuangpu|Sulawesi|Sibiti|Sanyo|Sanagawa|Serigawa|Shirotorigawa|Soezawaonnsenn| Soga|( Slew)$|(Stewart)$|Shigenobu")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Thalye)| tanukihaze|Tasmania|Tanabe|Tanegashima|Takeoka|( Tres Marias)|(Taiwanese)|Taiwan|Thai|Tocantins| Tobogan| Tominaga| Tristan|(Tristan)$|(Tristan )|Tlawng| Teramura|Tahiti| Takutu|Tamaki|Theriot|Tokai|Tiko|Torie|Taieri|Tadami|(Tentena Poso)|Terashimogawa|TSUMAGUROSUJIHAZE|Tsotzala|Turkey|Tokyo|Tochigi|Tomoegawa|Takagi|Takise")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Ural)$|(United Kingdom)|( Urocara)|( Ulimi)| Uruguay| Ucayali|UMass| Uppertas|USA")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Venezuela)|( Vanpoe)| Vevembe|Visayas|Vietnam|(Verde)$| Von Humboldt|Volga|Vanmar|Vanphi|Vanver|VietNam|Vladivostok")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, " Xingu|XChena")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Wainwright)$|Waimonga|Waimakariri|Waitaki|( Whitewater)|Watanabe|Wakayama|Wakamiyajinja|Wanquan|(Woods Hole) WThail|Wallagoattu|Washington|Waitahuna")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Yoma)|(Yalta)|YamasakiAizu|YamasakiNasu||Yamanasi|Yamanashi|Yoshino|( Yaku)$|Yahagigawa|Yahagi|Yamashita|Yamagata|( Zagno)|( Zarco)|( Zhange)|(Zhang)$|ZlZong| Ziyadi| Zhoushan|ZunVou| Zula|Zamoko|(Zeal)$|Zealand")) %>%
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  ...
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Ac)$|(Ansp)$|(Ait)$| Abrbra|( Ao)$|(Ara)$|(Ast)$|ARFu|ARQing|ARZhao|ARp|( ac)$|( Arg)$|( abri)$|( Abo)|( Asp )| Aab|( atg)$|( Ava)$|( aa)$|( ad)$|(Aa)$|(Ab)$|(Ac)$|(Al )|(Am )|(As)$|(Ain)$|(Amb)$| aaa|Axd|( Ala )|( ata)$| Afl|AcaSol|(Ala)$|(Ana)$|(Arg)$|(Asn)$|( Asn )|(Asp)$|(Aqu$)|( alpha)$|( Atl)$|(Avul)$|ASa|ASe|ASi|ASj|ASp|ASf|( azt)$|(Ant)$|(Aru)$|AcNif|(Apar)$|ALa|AsnCys|AsnTyr|( Asn )|( art)$|( atp)|AM|( alti)$|AcDu|AcNi|AcOl|AuMay")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Agr )|( art)$|AcTr|AcXa|( Adan)|Aasq|Agin|Aibe|( Asir)|Asoph|AspSahiler|AspKucuk|AspCavuscu|almsp")) %>%
  ...
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(CTo)$|( CrVb)|( Cr)$|(Cma)$|(Cp)$|(Cal)$|( Chab)|( Con)|( Ck)$|( Cw)$|chiTab|( chi)$|chiTro| chiPar|chiBor|chiHum|chiPzn|chiGup|chiMPur|chiPar|Cytb|cypn|(Cobs)$|( Cox)$|(Cyb)$|( cox)$|( Cyt)$|(Cys )|( Cui)$| cc|( co)$|( cyb)$|(Chi)$|CBAf|CBSo| chit| Cc|( cb)$|Cmb|( cl)$|( cg)$|Cosalb|( cha)$|CBZe|Ctri|Cti|Cxan|( Chm)$|( cn)$|Churaumi|Ccul|caixa|(Cleu)$|Cleu | Carg|(Cape)$|CPKut")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( DaAm)$|( Dec)$|( dki)|( dkic)|( dkiart)$|( dkisp)|( Dloop)$|( Dja)$|( Dab)|( Dud)|dfob|( day)$| dd|( de)$|( da)$|( do)$|( do )|DGGE|( db)$|Dma|Dpunc|(Dr)$|Drb|Drcyto|Drd|(Dre)$|Dree|(Drh)$|(Dri)$|Drm|( dl)$|(Dty)$|Dcds|dbNflu|dNflu")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "EspEita|ECpfp|( Ev)|( Ex)$|( eral)$|( Elp)$| ( ee)$|Ezs|(Eaw)$|( elas)$|FSc| ff|Fsb|epacxx|FRet")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Glu)$|(Gcoxii)|( gr)$|( gr )|( Gp)$|Gbh| Glu| Gly|GPpl|( Hje)$| gg|galx| Gln|( Gob)$|( galac)$|Ggs|( Hsp)|(HAx)| His| Hap|HUMed| Hvd| hva| hh|(Ha)$|(Hap)$|(Hal)$|( hap)$|Hk||Hhai|Helch|Helcu|Havi|HBatp|HBaltic|( hwp)$|Href|(Gem)$|Gme|Gwa|Gdv|GMk|hgtl|(Hy)$|(Hn)$|(Hg)$|(Hup)$|(Hyu)$|(Hu)$|(Hue)$|Hgat|(Hgem)|Hmes|Hmcs|Hmas|Hmbs|(Hrr)$|(hrr)$|Hta|Href")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Ila)$| ihb| Ile |ICOX|( ii)$|( iii)$|Ilike|( iso)$|imarbh|( ind)$|JUZoo| jj|ISs|( isl)$| Ifasy|IDAtl")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "Lbh|(Liz)$|(Lis)$|(Li)$|(La)$|(Lb)$|(Lc)$|(Ld)$|(Le)$|(Lau)$|(Lab)$|( Leu )|(Laf)|(Lra)$|(Lnas)|( Leu)$|LLamb| Lsicculus| Lsic| Lvana| Lvanb|( like)$|( liz)$|( Lg)$|(Lu)$|(Lp)$|( lin)$|( lll)|LVo|( ll)$|(Lmt)$|(Luz)$|LaGEEvo|(Leus)$|(Lgu)$|( Lys )")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( jima)$|(Kba)|(Kch)|KAst|Kx|Korsp|( ks)$|( kun)$| kk|( kappa)$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Mu)$|(Met)$|(Med)$|( Met )|( mar)$|( Maen)|Mpol|Mste|MHgem|MHgat|MHup|( myr)$|(Ms)$|(Mir)$|(Myr)$|( mm)$|Mparis|( ma)$|( ma )|( mt)$|mtlab|Mla|Mlb|Mlc|Moct|(misc)$|( mtg)$")) %>%
   mutate(ncbi_sp = str_remove_all(ncbi_sp, "( No)|(No)$|( Na)$|( Na )|( Nb)|(NanHai)|Nrs|Npk|Nsv|( Neo)$|(Nbre)|(Nb)$|(Nsa)$|notetop|( note)$|( ndr)$|( nd)$| naar|( nn)$|( nu)$|NCAtl|( Nel)$|( Nial)$|(Nili)$|(Nipo)$|(Nive)$|( nb)$|( nba)$|( ni)$|( ND)|NJrep|NaHe|Nkwo|Nerspp")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Ol)|OLuo|( Ot)$|OTp|( orp)$|( oti)$|( Okh)$|(Opa)$|(Ock)$|Otrna|( omml )|Ommt| Ored|Odsp")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Phe)|( Pro)|(Pro)$|PAs|( Ple)$|( pl)$|( Peo)$|( pit)$|( pol)$|PWh|( pp)$|( Pr)$|(Pic)$( Pen)$|PAva|Pg|(Pt)$|( pk)$|Pstx| Pafl|Pugcur|( pk )|(Port)$|(Pins)$|Plfl|Plms|Psfl|Psms|Plnc|( pr)$|(Pric)$|(Pwhe)$|Psebra|(Ping)$|PHref|QuOma")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "RStrn|rabr|(rasm)$|Rcat|(rcat)$|( ry )( rsp)$|( reve)$||RKp|Rfal|Ryunosuke|( rfal)|(Roa)$|Rbr|Rjk|( rep)$|Rlat")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "rpsa|rpsb|( sa)$|( sb)$|( sn)$|( Ser)|( s)$|(St)$|  strix|( syk)$| sbar|( ss)$|( ssc)$|( st)$|( sin)$|Ssu|( Sivi)$|(solr)$|Sarsp|( sigma)$|Swate|(Stenu)$|Stemm|Staen|Srost|( seq)$|(Seq)$|(seq)$|(sequence)$|( Sep)$|(Ssp)$|srbh|( Shi)$|(Srp)$|ScoPlu|( saru)$|( smp)$|( Srps)|( s GZ)")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Tr)$|( trn)$|( trn )|(Tej)$|(Tec)$|( Te)$|( Tk)$|( Tm)$( Tf)$|( Tq)$|( Tag)|( Thr)|( Tht)$|( Thy )|( Thy)$|( Tru)$|( Tyr)$|(Trp)|(Tri)$|( Tag)$|( tons)$|( the)$|( Truw)$|(Twdate)( Tpra)$|( Tfla)$|( TrTp)$|( ThorAur)|( Tas)$|(TSA)|( tru)$|(Trp)$|Tlab|TGen|( trn)$|( to )|Tde|( Tyr )|TaApr")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( uu)|UFde|URoscu|( Univ)$|( Val)|Vhis| vv|(Va)$|Vxx|(Vdm)$|Vmel|Vinf|(Vly)$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( Wol)|( Wt)$|( Wah)$|( Wah )|Waa|(Wab)$|(Wag)$|(Was)$| ww| xx|(Xec)$|XCtr|Xnov|(Xip)$|(Xin)$|XXsp|( yon)$|(Yar)$|(Ying)$|(Yo)$|Ybh| yy| zz| Zgr|( Zop)$| Zzu| Zargo| Zgzun| Zgjahu| ZfedOk| Zzin| Zinzin|Zstr|Zinstr| Zp|Zprolarvae| Zphi| Zxan|( Zs)$| Zch| Zc|( zch)$|( zcm)|Zcar|Zhue|zjzj|ZHue|ZCar|ZGp")) %>%
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  # random nonsense
  mutate(ncbi_sp = str_remove(ncbi_sp, "CDzjhq|AWFSFa|SIOfacabcfdbc|SIOafddbdccaca|CBRGDpas|SIOfdacacbacdfdcdbSIOeffdeeabcfecc|SIOfdacacbacdfdcdb|SIOeffdeeabcfecc|tiKamakonde|SCSEbi|SIOfbacdaadfdb|GoYYJia|GrLar|SIObbafaaefbfeeff|XMHoceano")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "ratrataluspat|caebaabbfdecb|deccebbcfeceae|bfeeddfdffbafc|bddbcebdeabeb|dRybinskoye|ceceeabf|cdebebddfcbaea|fdccbbad|ffafbacdabbeedfb|ffaebabfabbdf|familynameLoricariidae|Ncytidinemettl|ddfdfbecafbafdc|svcimar|eafabebccbadebdca|aebfbcafbebdbd|fbfbaeeaefedbdcf|mizuhikinagamimizuhaze|bakemimizuhaze|gomamimizuhaze|aeeedcbdceddcde|ebbdfbbacb|bfabdeddfaf|SIObdeadbaacccbacb|SIOcbedecefccf|CDzjydj|CDzjbdj|NMAro|SIOcbaafcae|iolalali|SIOaccedfbd|parial ura|PfILFTriplExLD|omV hr LPS|TPAasm|HERMcatBullseye| withpart  at  side|ChromBimbia|SIOfaddafbafcedf|Htrbsamordcidkrdnz|Hsamcidkrdnz|SIOcbbdbefff|ELRAQZhang|ELDGQZhang|sMCFab|sMBFab")) %>%
  #GO AWAY
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "(Fish)$|(FISHCARD)|(Lake)$|(Chiba)$|CASIch|GADAR|( trn)$|(Rocha)$|Mediterranean|Chitose|MABIK|MZict| Asn|aavariant|(S     I   II F   F)|(Eslu)$|Yoshino|TuDec")) %>%
  ...
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  # stragglers
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "( chi)$|( Lg)$|( like)$|( Lakshadweep)|( Geo)$|( chit)$|( scute)$|( bn)$|( Malawi)|yatremusluteus|( Sea)|( Bim)$|USNMFish|(Aa)$|(Ab)$|(Ac)$|(As)$|daCaspianSea|( bl)$|ZMUCFisk|MCZIch|( fish)$|(Fish)$|NakamuraIDEAFish|Ichthyology|Ichthyological|AzovSea|CBMChiba|(Tyr)$|(Arg)$|( Hyge)|Mnegrosensis|Magnisudisatlantica|(complete)$|CaliniaPendleton|DFQAMINDTNTUTYAuxisThrespuram|( miu)$|( eral)$|HBatp|( rep)$|Yamanashi|MZUSPskel|(S     I   II F)|Ilong|(Asn)$|( val)$|( psm)$|( art)$|(CPOG  Acostrica)|phenotypepe|( elas)$")) %>%
  mutate(ncbi_sp = trimws(ncbi_sp)) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  mutate(ncbi_sp = str_remove_all(ncbi_sp, "([A-Z])$")) %>%
  ...
  mutate(ncbi_sp = str_replace_all(ncbi_sp, 
                                   c('albacaresfish' = 'albacares',
                                     ...
                                     # 'for' errors
                                     'Zcalinianus' = 'Zalophus calinianus',
                                     'caliniensis' = 'californiensis',
                                     'calinianus' = 'californianus',
                                     'calinica' = 'californica',
                                     'calinicus' = 'californicus',
                                     'Bajacalinia' = 'Bajacalifornia',
                                     # 'land' error
                                     'Clevelia' = 'Clevelandia',
                                     'Seriola lali' = 'Seriola lalandi',
                                     # stri? ugh
                                     'Atypichthysgatus' = 'Atypichthys strigatus',
                                     'Aeoliscusgatus' = 'Aeoliscus strigatus',
                                     'Aphyosemionatum' = 'Aphyosemion striatum',
                                     'Antennariusatus' = 'Antennarius striatus',
                                     'Botiaata' = 'Botia striata',
                                     'Valencienneagata' = 'Valenciennea strigata',
                                     'Centropristisata' = 'Centropristis striata',
                                     'Chaetodonatus' = 'Chaetodon striatus',
                                     'Channaata' = 'Channa striata',
                                     'Cobitisata' = 'Cobitis striata',
                                     'Ctenochaetusatus' = 'Ctenochaetus striatus',
                                     'Ctenochaetusgosus' = 'Ctenochaetus strigosus',
                                     'Dermatolepisolata' = 'Dermatolepis striolata',
                                     'Dimidiochromisgatus' = 'Dimidiochromis strigatus',
                                     'Entomacrodusatus' = 'Entomacrodus striatus',
                                     'Epinephelusatus' = 'Epinephelus striatus',
                                     'Guinardiaata' = 'Guinardia striata',
                                     'Helcogrammaata' = 'Helcogramma striata',
                                     'Hymenocephalusatissimus' = 'Hymenocephalus striatissimus',
                                     'Iconisemionatum' = 'Iconisemion striatum',
                                     'Ilishaatula' = 'Ilisha striatula',
                                     'Jaydiaata' = 'Jaydia striata',
                                     'Jaydiaatodes' = 'Jaydia striatodes',
                                     'Laimosemiongatum' = 'Laimosemion strigatum',
                                     'Leporinusatus' = 'Leporinus striatus',
                                     'Luciocyprinusolatus' = 'Luciocyprinus striolatus',
                                     'Microcanthusgatus' = 'Microcanthus strigatus',
                                     'Parapercisolata' = 'Parapercis striolata',
                                     'Paratherinaata' = 'Paratherina striata',
                                     'Pomadasysdens' = 'Pomadasys stridens',
                                     'Praealticusatus' = 'Praealticus striatus',
                                     'Pristobryconolatus' = 'Pristobrycon striolatus',
                                     'Pseudotriacanthusgilifer' = 'Pseudotriacanthus strigilifer',
                                     'Rineloricariagilata' = 'Rineloricaria strigilata',
                                     'Stethojulisgiventer' = 'Stethojulis strigiventer',
                                     'Trachelyopterusatulus' = 'Trachelyopterus striatulus',
                                     'Trimmaatum' = 'Trimma striatum',
                                     # maybe too dangerous to str_remove
                                     'chalumnaecoelacanth' = 'chalumnae',
                                     'oligacanthusrthwest' = 'oligacanthus',
                                     'vLadykovi' = 'vladykovi',
                                     'Eigenmannia virescens  Eigenmannia virescens' = 'Eigenmannia virescens',
                                     # var
                                     'carpio var baisenensis' = 'carpio',
                                     'minuta var cassieae' = 'minuta',
                                     'cinnamomi var cinnamomi' = 'cinnamomi',
                                     # outgroups
                                     'Unculturedospirillaceae' = 'Oceanospirillaceae'
                                     ))) 
